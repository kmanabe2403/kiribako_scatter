...
// 指定した長さの直線を描く関数
void draw_limited_line(cv::Mat& img, cv::Point p1, cv::Point p2, int length) {
    // 2点間の距離を計算
    double dx = p2.x - p1.x;
    double dy = p2.y - p1.y;
    ...
    if (distance > 0) {
        ...
        cv::Point p_end(p1.x + static_cast<int>(dx * scale), 
...
void get_coordinates(int event, int x, int y, int flags, void* param) {
    if (event == cv::EVENT_LBUTTONDOWN) {  // 左クリック時
        ...
        if (click_count % 2 == 1) {
            ...
            draw_limited_line(*img, point1, point2, 500);  // 500ピクセルぶんの直線を描く
        }
