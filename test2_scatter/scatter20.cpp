...
std::vector<cv::Point> result; //なぜかこいつがいないとpoints2がclickごとに引き継がれなくなってしまう...
...
cv::Mat img_original;  // 読み込んだ画像
cv::Point2f offset(0, 0);  // 画像の表示位置オフセット
double scale = 1.0;  // 画像の拡大縮小スケール
const int MOVE_STEP = 20;  // 矢印キーで移動するピクセル量
const double SCALE_STEP = 1.1;  // 拡大縮小の倍率

// 画像の表示更新
void update_display() {
    if (img_original.empty()) return;

    // スケール適用
    cv::Mat resized_img;
    cv::resize(img_original, resized_img, cv::Size(), scale, scale);

    // オフセット適用
    int roi_x = std::max(0, static_cast<int>(offset.x));
    int roi_y = std::max(0, static_cast<int>(offset.y));
    int roi_width = std::min(resized_img.cols - roi_x, img_original.cols);
    int roi_height = std::min(resized_img.rows - roi_y, img_original.rows);

    // ROIサイズが1以上か確認
    if (roi_width <= 0 || roi_height <= 0) return;

    cv::Mat cropped = resized_img(cv::Rect(roi_x, roi_y, roi_width, roi_height));
    cv::imshow("img_bin", cropped);
}
...
//         is_dragging = true;
//         prev_mouse_pos = cv::Point(x, y);
//     } else if (event == cv::EVENT_MOUSEMOVE && is_dragging) {
//         offset += cv::Point2f(x - prev_mouse_pos.x, y - prev_mouse_pos.y);
//         prev_mouse_pos = cv::Point(x, y);
//         update_display();
//     } else if (event == cv::EVENT_LBUTTONUP) {
//         is_dragging = false;
//     } else if (event == cv::EVENT_MOUSEWHEEL) {
//         int delta = (flags > 0) ? 1 : -1;
//         scale *= (1.0 + 0.1 * delta);
//         update_display();
//     }
// }
...
int main() {
    ...
    name_in << "./example.png";  // 読み込む画像のファイル名

    img_original = cv::imread(name_in.str());
    if (img_original.empty()) {
        std::cerr << "Error: Could not load image!" << std::endl;
        return -1;
    }

    cv::namedWindow("img_bin", cv::WINDOW_AUTOSIZE);
    update_display();  // 初回表示
    ...
    while (true) {
        ...
        if (key == 81) {  // ← 左
            offset.x = std::max(0.0f, offset.x - MOVE_STEP);
        } else if (key == 83) {  // → 右
            offset.x = std::min(static_cast<float>(img_original.cols), offset.x + MOVE_STEP);
        } else if (key == 82) {  // ↑ 上
            offset.y = std::max(0.0f, offset.y - MOVE_STEP);
        } else if (key == 84) {  // ↓ 下
            offset.y = std::min(static_cast<float>(img_original.rows), offset.y + MOVE_STEP);
        } else if (key == 43 || key == 61) {  // +キー (拡大)
            scale *= SCALE_STEP;
        } else if (key == 45) {  // -キー (縮小)
            scale = std::max(0.1, scale / SCALE_STEP);
        } else if (key == 27) {  // ESCキーで終了
            break;
        } else if (key == 's') {  // 's'キーで画像保存
            cv::imwrite("output.png", img_original);
        }
