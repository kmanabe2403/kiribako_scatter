...
void draw_line(cv::Mat& img, cv::Point p1, cv::Point p2, int length) {
    double dx = p2.x - p1.x;
    double dy = p2.y - p1.y;
    ...
    if (distance > 0) {
        ...
        cv::line(img, p1, p_end, cv::Scalar(0,0,255), 2);
    }
}

//ある長さの線を引く(distanceぶん)
void draw_line2(cv::Mat& img, cv::Point p1, cv::Point p2){
    double dx = p2.x - p1.x;
    double dy = p2.y - p1.y;
    double distance = std::sqrt(dx*dx + dy*dy);
    cv::Point p_end = p1;
...
std::tuple<cv::Point, cv::Point> draw_line3(cv::Mat& img, cv::Point p1, cv::Point p2, int length) {
    double dx = p2.x - p1.x;
    double dy = p2.y - p1.y;
    double distance = std::sqrt(dx*dx + dy*dy);
    cv::Point p_med = p1;
...
void draw_subline(cv::Mat& img, cv::Point p_int, cv::Point p1, cv::Point p2, int length) {
    double dx = p2.x - p1.x;
    double dy = p2.y - p1.y;
    double distance = std::sqrt(dx*dx + dy*dy);
    double dx_ver = -dy;
    ...
    if (distance > 0) {
        ...
        cv::line(img, p_int, p_end, cv::Scalar(110, 110, 110), 1);
    }
}
...
void at_click(int event, int x, int y, int flags, void* param) {
    if (event == cv::EVENT_LBUTTONDOWN) {  // 左クリック時
        ...
        if (click == 1) {
            ...
            point2 = cv::Point(x,y);
            std::tie(point2_2, point2_3) = draw_line3(*img, point1, point2, 500);
            cv::circle(*img, point2_2, 3, cv::Scalar(0,0,255), -1);
            cv::circle(*img, point2_3, 3, cv::Scalar(0,0,255), -1);
            draw_subline(*img, point2_2, point1, point2, 500);
            draw_subline(*img, point2_3, point1, point2, 500);
        } else if (click == 3) {
            point3 = cv::Point(x,y);
            draw_line2(*img, point2_3, point3);
            draw_line2(*img, point2, point3);
            draw_subline(*img, point3, point2_3, point3, -200);
        } else if (click == 4) {
            point4 = cv::Point(x,y);
            draw_line2(*img, point2_2, point4);
            draw_line2(*img, point3, point4);
            draw_line(*img, point2, point4, 500);
        } else if (click == 5) {
            point5 = cv::Point(x,y);
            draw_line2(*img, point3, point5);
        } else {
            click = 1;
            point1 = cv::Point(x,y);
        }
...
int main() {
    ...
    while (true) {
        ...
        if (key == 's') {  // 's'キーが押されたら
            cv::imwrite("beta1_scatter6.png", img_color);  // 画像を保存
