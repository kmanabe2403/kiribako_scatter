...
void at_click(int event, int x, int y, int flags, void* param) {
    if (event == cv::EVENT_LBUTTONDOWN) {  // 左クリック時
        ...
        if (click == 1) {
            ...
            cv::circle(*img, cv::Point(x, y), 20, cv::Scalar(255, 0, 0), 1); //幅20の円を書く
            ...
            std::tie(point2_2, point2_3, a0) = draw_line3(*img, point1, point2, 200, cv::Scalar(100, 100, 255), 1);
            ...
            if (key == '1') {  // 1が押されたら、point3->point2_3に対して右側に線を引く
                a1 = draw_subline(*img, point3, point3, point2_3, 100, cv::Scalar(100, 100, 255), 1);
            ...
            a3 = draw_line(*img, point2, point4, 200, cv::Scalar(100, 100, 255), 1);
...
int main() {
    ...
    while (true) {
        int key = cv::waitKey(0);  // キー入力を待つ
        if (key == 's') {  // 's'キーが押されたら
            cv::imwrite("example_scatter10.png", img_color);  // 画像を保存
