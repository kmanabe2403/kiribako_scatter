...
void at_click(int event, int x, int y, int flags, void* param) {
    if (event == cv::EVENT_LBUTTONDOWN) {  // 左クリック時
        ...
        if (click == 1) {
            ...
            point2 = cv::Point(x,y);
            std::tie(point2_2, point2_3, a0) = draw_line3(*img, point1, point2, 500, cv::Scalar(100, 100, 255), 3);
            ...
            a1 = draw_subline(*img, point3, point3, point2_3, 200, cv::Scalar(100, 100, 255), 3);
        } else if (click == 4) {
            point4 = cv::Point(x,y);
            draw_line2(*img, point2_2, point4, cv::Scalar(100, 100, 100), 1);
            a2 = draw_line2(*img, point4, point3, cv::Scalar(100, 100, 255), 3);
            a3 = draw_line(*img, point2, point4, 500, cv::Scalar(100, 100, 255), 3);
        } else if (click == 5) {
            point5 = cv::Point(x,y);
...
int main() {
    ...
    while (true) {
        ...
        if (key == 's') {  // 's'キーが押されたら
            cv::imwrite("beta1_scatter8.png", img_color);  // 画像を保存
